...
const EMPTY_FEE: FeeForm = { type: "TUITION", name: "", amount: "", isActive: true, paymentDestinationId: "" };
const EMPTY_DESTINATION: DestinationForm = { label: "", providerCode: "", accountName: "", accountNumber: "", phoneNumber: "" };
...
export default function FeesPage() {
  ...
  const addDestination = useMutation({
    mutationFn: () =>
      schoolService.addPaymentDestination(schoolId, {
    ...
    onSuccess: () => {
      ...
      toast({ title: "Payment destination added", variant: "success" });
    },
    onError: (e) => toast({ title: "Could not add destination", description: (e as unknown as ApiError).message, variant: "error" }),
  });
  ...
  return (
    <PageTransition>
          ...
          {!loadingFees && fees.length === 0 ? (
            ...
            <DataTable<RealSchoolFee>
              ...
              columns={[
                ...
                { key: "type", header: "Type", render: (f) => FEE_TYPE_LABEL[f.type] },
                {
                  key: "isActive",
                  header: "Status",
              ...
              onRowClick={
                has(P.FEES_CONFIGURE)
                  ? (f) => setFeeForm({
                      ...
                      isActive: f.isActive ?? true, paymentDestinationId: f.paymentDestinationId ?? "",
                    })
                  : undefined
              }
            ...
            <Card padded={false}>
              ...
              {loadingDestinations ? (
                ...
                <div className="divide-y divide-line">
                  {destinations.map((d: PaymentDestination) => {
                    ...
                    return (
                      <div key={d.id} className="flex items-center gap-3 px-4 py-2.5">
                        ...
                        <Badge variant={d.isActive ? "success" : "neutral"} dot>{d.isActive ? "Active" : "Inactive"}</Badge>
                      </div>
                    );
                  })}
                </div>
        ...
        {feeForm && (
          <div className="space-y-3.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Select label="Type" value={feeForm.type} onChange={(e) => setFeeForm({ ...feeForm, type: e.target.value as RealFeeInput["type"] })}>
      ...
      <Modal
        ...
        onClose={() => !addDestination.isPending && setDestinationForm(null)}
        title="Add payment destination"
        footer={
          <>
            <Button variant="ghost" onClick={() => setDestinationForm(null)} disabled={addDestination.isPending}>Cancel</Button>
            <Button
              loading={addDestination.isPending}
              disabled={!destinationForm?.label.trim() || !destinationForm?.providerCode || !destinationForm?.accountName.trim()}
