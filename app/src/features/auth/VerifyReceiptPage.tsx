...
import { paymentService, type VerifiedReceipt } from "@/services/paymentService";
import { formatDateTime } from "@/lib/format";

/**
 * Public anti-fraud tool: anyone holding a paper/PDF receipt can confirm it was really issued
 * by a school on E-SHURI by entering its reference code. GET /receipts/verify/:reference.
 */
export default function VerifyReceiptPage() {
  const [reference, setReference] = useState("");
  const [result, setResult] = useState<VerifiedReceipt | null | "none">("none");
  const [loading, setLoading] = useState(false);

  const onSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const receipt = await paymentService.verifyReceipt(reference.trim());
    setResult(receipt);
    setLoading(false);
  };
  ...
  return (
    <AuthLayout>
      ...
      <p className="text-muted text-[14px] mt-1 mb-7">
        Enter the reference code printed on any E-SHURI receipt (e.g. <span className="tnum font-medium">ESH-2026-XXXXXXXX</span>)
        to confirm it's genuine.
      </p>

      <form onSubmit={onSubmit} className="flex gap-2">
        <Input
          placeholder="ESH-2026-XXXXXXXX"
          value={reference}
          ...
          {result ? (
            <div className="rounded-(--radius-card) border border-primary/40 bg-primary-soft/50 p-5">
              ...
              <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2.5 text-[13.5px]">
                ...
                <div><dt className="text-muted">Amount</dt><dd className="font-semibold text-ink tnum">{result.currency} {result.amount.toLocaleString()}</dd></div>
                ...
                <div className="col-span-2"><dt className="text-muted">For</dt><dd className="font-medium text-ink">{result.feeName}</dd></div>
                <div className="col-span-2"><dt className="text-muted">Issued</dt><dd className="font-medium text-ink">{formatDateTime(result.paidAt)}</dd></div>
              </dl>
            </div>
          ) : (
